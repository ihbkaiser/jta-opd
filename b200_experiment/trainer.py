...
def _opd_train_step(
    ...
    for relative_index, ppo_index in enumerate(range(offset, offset + limit)):
        ...
        cmt_branch_started = time.perf_counter()
        cmt_result = _opd_train_step_impl(**common_group)
        ...
        try:
            ...
            one_step_probe.write_pair(
                ...
                cmt_eval_time_sec=float(time.perf_counter() - cmt_branch_started),
            )
        except BaseException as error:
...
def run_training(
    ...
    setup_progress.set_postfix_str("stage=trajectory-kl-probe", refresh=True)
        ...
        final_metrics = {
            ...
            * num_responses,
            ...
            "one_step_kl_probe_subset_size": (
                one_step_probe.config.subset_size
                if one_step_probe is not None
                else None
            ),
            "one_step_kl_probe_rollouts_per_problem": (
                one_step_probe.config.num_rollouts_per_problem
                if one_step_probe is not None
                else None
            ),
            "one_step_kl_probe_horizon": (
                one_step_probe.config.horizon
                if one_step_probe is not None
                else None
            ),
            "micro_batch_size_per_gpu": micro_batch_size_per_gpu,
