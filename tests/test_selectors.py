...
class SelectorTests(unittest.TestCase):
    ...
    def test_ta_matches_literal_union_kl_and_teacher_mass_definition(self):
        ...
        self.assertEqual(
            output.diagnostics["support_definition"],
    ...
    def test_ta_compact_path_uses_teacher_only_topk_tokens(self):
        student_logits = self.p.log()
        teacher_logits = self.q.log()
        student_ids = torch.topk(student_logits, 8, dim=-1).indices
        teacher_ids = torch.topk(teacher_logits, 8, dim=-1).indices
        ...
        self.assertFalse(
            torch.allclose(actual.diagnostics["D"], reference.diagnostics["D"])
        )
        # Compatibility is defined only by teacher mass on Student Top-K.
        self.assertTrue(
            torch.allclose(actual.diagnostics["C"], reference.diagnostics["C"])
        )
