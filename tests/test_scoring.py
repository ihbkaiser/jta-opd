...
def test_joint_full_vocab_metrics_match_direct_distributions():
    ...
    teacher = torch.tensor([[[0.0, 1.0, -0.5], [0.4, -0.3, 0.8]]])
    student_z = torch.logsumexp(student, dim=-1)
    teacher_z = torch.logsumexp(teacher, dim=-1)
    mean, variance, common = _joint_full_vocab_metrics(
        student,
        teacher,
        student_z,
        teacher_z,
    ...
    log_q = torch.log_softmax(teacher, dim=-1)
    p, q = log_p.exp(), log_q.exp()
    r = log_q - log_p
    expected_mean = (p * r).sum(dim=-1)
    expected_variance = (p * (r - expected_mean.unsqueeze(-1)).square()).sum(dim=-1)
    assert torch.allclose(mean, expected_mean, atol=1e-6)
    assert torch.allclose(variance, expected_variance, atol=1e-6)
    assert torch.allclose(common, torch.minimum(p, q).sum(dim=-1), atol=1e-6)
...
class ScoringTests(unittest.TestCase):
    ...
    def test_joint_bidirectional_scoring_matches_three_independent_forwards(self):
        ...
        diagnostic_student, _ = score_student_teacher_rollout(
            _RecordingQwenModel(student_logits),
            _RecordingQwenModel(teacher_logits),
            rollout,
            top_k=5,
        ...
        for metric in (
            diagnostic_student.full_log_ratio_mean,
            diagnostic_student.full_log_ratio_variance,
            diagnostic_student.full_common_mass,
        ):
            self.assertIsNotNone(metric)
            self.assertTrue(torch.isfinite(metric[valid]).all())
