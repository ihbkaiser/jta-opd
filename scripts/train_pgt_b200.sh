...
export RUN_NAME="${RUN_NAME:-pgt_$(date +%Y%m%d_%H%M%S_%N)}"
...
export TOKEN_BUDGET_RHO="${PGT_RHO}"
...
if [[ -n "${RESUME_FROM_CHECKPOINT}" && "${RESUME_FROM_CHECKPOINT}" != "auto" && -z "${OUTPUT_DIR:-}" ]]; then
  ...
  OUTPUT_DIR="${OUTPUT_DIR:-${PGT_RUN_OUTPUT}}"
fi
build_training_args "${OUTPUT_DIR}"
echo "PGT run: ${RUN_NAME}"
echo "PGT output: ${OUTPUT_DIR}"
...
run_training_cli train \
  ...
  "${COMMON_TRAIN_ARGS[@]}" "$@"
if [[ -n "${RESUME_FROM_CHECKPOINT}" ]]; then
  echo "PGT resume completed in: ${OUTPUT_DIR}"
else
  echo "PGT completed. Keep this identifier: PGT_RUN_NAME=${RUN_NAME}"
fi
