...
case "${TRAIN_DATASET,,}" in
  ...
  competition_math|competition-math|math)
    ...
    _DEFAULT_TRAIN_DATA_SPLIT="null"
    _DEFAULT_PROMPT_KEY="problem"
    ;;
  *)
    _DEFAULT_TRAIN_DATA="${TRAIN_DATA:-}"
    _DEFAULT_TRAIN_DATA_SPLIT="${TRAIN_DATA_SPLIT:-null}"
    _DEFAULT_PROMPT_KEY="${PROMPT_KEY:-prompt}"
    ;;
esac
export TRAIN_DATA="${TRAIN_DATA:-${_DEFAULT_TRAIN_DATA}}"
export TRAIN_DATA_SPLIT="${TRAIN_DATA_SPLIT:-${_DEFAULT_TRAIN_DATA_SPLIT}}"
export PROMPT_KEY="${PROMPT_KEY:-${_DEFAULT_PROMPT_KEY}}"
...
export REEVAL_NUM_RESPONSES="${REEVAL_NUM_RESPONSES:-8}"
export REEVAL_MAX_NEW_TOKENS="${REEVAL_MAX_NEW_TOKENS:-8192}"
export REEVAL_VLLM_MAX_MODEL_LEN="${REEVAL_VLLM_MAX_MODEL_LEN:-9216}"
export REEVAL_VLLM_GPU_WORKSPACE_HEADROOM_GIB="${REEVAL_VLLM_GPU_WORKSPACE_HEADROOM_GIB:-2}"
