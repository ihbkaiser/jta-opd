...
if [[ -z "${RUN_NAME:-}" ]]; then
  _cmt_fraction_tag="${CMT_TOP_FRACTION//./p}"
  _cmt_fraction_tag="${_cmt_fraction_tag//-/m}"
  RUN_NAME="cmt_${CMT_ALLOCATION_MODE}_f${_cmt_fraction_tag}_$(date +%Y%m%d_%H%M%S_%N)"
fi
export RUN_NAME
