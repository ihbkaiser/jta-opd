...
build_training_args() {
  ...
  COMMON_TRAIN_ARGS=(
    ...
    --set "one_step_kl_probe.parent_state_count=${ONE_STEP_KL_PROBE_PARENT_STATE_COUNT:-64}"
    --set "one_step_kl_probe.successors_per_parent=${ONE_STEP_KL_PROBE_SUCCESSORS_PER_PARENT:-1}"
    --set "one_step_kl_probe.seed=${ONE_STEP_KL_PROBE_SEED:-20260922}"
    --set "one_step_kl_probe.interval_steps=${ONE_STEP_KL_PROBE_INTERVAL:-10}"
    --set "one_step_kl_probe.sampling_temperature=${ONE_STEP_KL_PROBE_SAMPLING_TEMPERATURE:-1.0}"
    --set "one_step_kl_probe.score_micro_batch_size=${ONE_STEP_KL_PROBE_SCORE_MICRO_BATCH_SIZE:-8}"
