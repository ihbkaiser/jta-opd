...
build_training_args() {
  ...
  COMMON_TRAIN_ARGS=(
    ...
    --set "selector.cmt_allocation_mode=${CMT_ALLOCATION_MODE:-gibbs}"
    --set "selector.cmt_top_fraction=${CMT_TOP_FRACTION:-0.10}"
    ...
    --set "logging.token_score_raw_sample_size=${TOKEN_SCORE_RAW_SAMPLE_SIZE:-8192}"
    --set "logging.token_score_histogram_bins=${TOKEN_SCORE_HISTOGRAM_BINS:-256}"
    --set "logging.cmt_detailed_log_enabled=${CMT_DETAILED_LOG_ENABLED:-false}"
    --set "logging.cmt_tail_log_enabled=${CMT_TAIL_LOG_ENABLED:-false}"
    --set "logging.cmt_tail_log_interval=${CMT_TAIL_LOG_INTERVAL:-1}"
    --set "logging.cmt_tail_top_k=${CMT_TAIL_TOP_K:-128}"
    --set "logging.cmt_tail_context_tokens=${CMT_TAIL_CONTEXT_TOKENS:-16}"
    --set "logging.cmt_tail_fp64_check=${CMT_TAIL_FP64_CHECK:-true}"
    --set "logging.cmt_partial_horizons=${CMT_PARTIAL_HORIZONS:-[16,64,256,1024]}"
