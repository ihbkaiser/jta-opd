...
build_training_args() {
  ...
  COMMON_TRAIN_ARGS=(
    ...
    --set "selector.cmt_allocation_mode=${CMT_ALLOCATION_MODE:-direct_bounded_gibbs}"
    --set "selector.cmt_weight_min=${CMT_WEIGHT_MIN:-0.5}"
    --set "selector.cmt_weight_max=${CMT_WEIGHT_MAX:-2.0}"
    --set "selector.cmt_correction_mode=${CMT_CORRECTION_MODE:-tanh_q99}"
    --set "selector.cmt_correction_quantile=${CMT_CORRECTION_QUANTILE:-0.99}"
    --set "selector.cmt_final_allocation_kl=${CMT_FINAL_ALLOCATION_KL:-0.02}"
    ...
    --set "one_step_kl_probe.enabled=${ONE_STEP_KL_PROBE_ENABLED:-false}"
    --set "one_step_kl_probe.benchmark=${ONE_STEP_KL_PROBE_BENCHMARK:-Competition-MATH}"
    --set "one_step_kl_probe.subset_size=${ONE_STEP_KL_PROBE_SUBSET_SIZE:-64}"
    --set "one_step_kl_probe.num_rollouts_per_problem=${ONE_STEP_KL_PROBE_NUM_ROLLOUTS_PER_PROBLEM:-2}"
    --set "one_step_kl_probe.horizon=${ONE_STEP_KL_PROBE_HORIZON:-64}"
    --set "one_step_kl_probe.seed=${ONE_STEP_KL_PROBE_SEED:-20260923}"
    --set "one_step_kl_probe.interval_steps=${ONE_STEP_KL_PROBE_INTERVAL:-50}"
    --set "one_step_kl_probe.sampling_temperature=${ONE_STEP_KL_PROBE_SAMPLING_TEMPERATURE:-1.0}"
    --set "one_step_kl_probe.sampling_top_p=${ONE_STEP_KL_PROBE_SAMPLING_TOP_P:-1.0}"
    --set "one_step_kl_probe.generation_batch_size=${ONE_STEP_KL_PROBE_GENERATION_BATCH_SIZE:-8}"
    --set "one_step_kl_probe.score_micro_batch_size=${ONE_STEP_KL_PROBE_SCORE_MICRO_BATCH_SIZE:-1}"
    ...
    --set "logging.cmt_token_audit_enabled=${CMT_TOKEN_AUDIT_ENABLED:-false}"
    --set "logging.cmt_token_audit_interval=${CMT_TOKEN_AUDIT_INTERVAL:-150}"
    --set "logging.cmt_token_audit_top_k=${CMT_TOKEN_AUDIT_TOP_K:-50}"
    --set "logging.cmt_token_context_radius=${CMT_TOKEN_CONTEXT_RADIUS:-32}"
    --set "logging.cmt_gain_heatmap_enabled=${CMT_GAIN_HEATMAP_ENABLED:-false}"
