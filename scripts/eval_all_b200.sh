...
JTA_EVAL_OUTPUT="${JTA_EVAL_OUTPUT:-${RUN_RESULTS_DIR}/eval/jta_opd}"
...
JTA_CHECKPOINT="${JTA_CHECKPOINT:-${JTA_RUN_OUTPUT}/final}"
...
RUN_JTA_EVAL="${RUN_JTA_EVAL:-false}"
...
if [[ "${RUN_JTA_EVAL}" == "true" ]]; then
  RUN_NAME="${RUN_NAME}" RESULTS_DIR="${RUN_RESULTS_DIR}" \
    JTA_OUTPUT_DIR="${JTA_RUN_OUTPUT}" JTA_CHECKPOINT="${JTA_CHECKPOINT}" \
    JTA_EVAL_OUTPUT="${JTA_EVAL_OUTPUT}" \
    bash "${SCRIPT_DIR}/eval_jta_b200.sh"
fi
AGGREGATE_ARGS=(
...
if [[ "${RUN_JTA_EVAL}" == "true" ]]; then
  AGGREGATE_ARGS+=(--jta-dir "${JTA_EVAL_OUTPUT}")
fi
